...
! now read
  ...
  if (velocity) then
     ipad1=int(-50.*FWHM*lambda(1)/2.99792458e5/dlam)+1
     ipad2=int(-50.*FWHM*lambda(isize)/2.99792458e5/dlam2)+1
  else
...
! loop to convolve 4 spectra (if needed)
  do itype=1,jmax
    ...
    do ii=1,isize,resample
     ...
     if (velocity) then
       FWHM2=-FWHM*lambda(ii)/2.99792458e5 ! km/s -> A
       ipad=int(-50.*FWHM*lambda(ii)/2.99792458e5/dlam)+1
     else
